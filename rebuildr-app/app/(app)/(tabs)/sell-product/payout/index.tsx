      ...
      onCompleted: (data) => {
        timer = setInterval(async () => {
          ...
          if (
            ...
            router.push("/(app)/sell-product/payout/payout-method");
          }
          if (
            authenticationRes?.authenticateRocker.status ===
        ...
        Linking.openURL(
          `bankid:///?autostarttoken=${data.authenticateRocker.autoStartToken}&redirect=lofta:///account/payout/verify`,
        );
      },
      onError: () => {
        clearInterval(timer);
      ...
      <View style={{ gap: 24 }}>
          ...
          {showQr ? (
            <BankId
              ...
              onAuthenticationSuccess={() => {
                setShowQr(false);
                router.navigate("/(app)/sell-product/payout/payout-method");
              }}
            />
          ) : (
        ...
        <Display size="small" style={{ textAlign: "center" }}>
          {showQr
            ...
            : "Börja med att koppla ett utbetalningskonto"}
        </Display>
        <Body size="medium" style={{ textAlign: "center" }}>
          {showQr
            ? "Du verkar inte ha kopplat något utbetalningskonto ännu. För att få betalt behöver du först verifiera dig med BankID."
            : "Du har inget utbetalningskonto kopplat. För att få betalt, verifiera dig med BankID och välj sedan Swish eller bankkonto via Trustly."}
        </Body>
        {showQr && <Divider />}
        {showQr && (
          <View style={{ gap: 16 }}>
