import {
  ...
  SellProductBottomSheetQueryQuery,
  File as GqlFile,
  ProductStatusEnum,
  SellProductUpdateDraftMutation,
  SellProductUpdateDraftMutationVariables,
} from "@/gql/graphql";
...
import { HandleDraftBottomSheet } from "./handle-draft-bottom-sheet";
import { SELL_PRODUCT_BOTTOM_SHEET_PRODUCT_FRAGMENT } from "./queries";
...
export const SELL_PRODUCT_BOTTOM_SHEET_QUERY = gql`
  query SellProductBottomSheetQuery {
    getOrCreateDraftProduct {
      ...SellProductBottomSheetProductFragment
    }
    me {
      id
  ...
  ${SELL_PRODUCT_BOTTOM_SHEET_PRODUCT_FRAGMENT}
`;

const SELL_PRODUCT_UPDATE_DRAFT = gql`
  mutation SellProductUpdateDraft($input: UpdateProductInput!) {
...
export const SellProductBottomSheet = () => {
  ...
  const [updateDraft, { loading: updatingDraft }] = useMutation<
    SellProductUpdateDraftMutation,
    SellProductUpdateDraftMutationVariables
  >(SELL_PRODUCT_UPDATE_DRAFT);
  ...
  useEffect(() => {
    const productToState = async (
      dbProduct: NonNullable<
        SellProductBottomSheetQueryQuery["getOrCreateDraftProduct"]
      >,
    ) => {
    ...
    if (data) {
      ...
      productToState(data.getOrCreateDraftProduct);
    }
  }, [data]);
  ...
  const onSave = (published: boolean) => {
    ...
    updateDraft({
      variables: {
        input: {
          id: data.getOrCreateDraftProduct.id,
          title: product.title,
          ...
          addImages: addFiles(
            product.images,
            data.getOrCreateDraftProduct.images,
          ),
          removeImages: removeFiles(
            product.images,
            data.getOrCreateDraftProduct.images,
          ),
          addDocuments: addFiles(
            product.documents,
            data.getOrCreateDraftProduct.documents,
          ),
  ...
  const updateDraftLoading = updatingDraft || uploadingMedia;
  ...
  return (
    <BottomSheet
      ...
      header={
        <ProgressHeader
          prog1={step === "payout" ? 25 : progressDetails()}
          ...
          title={step === "preview" ? "Förhandsgranska annons" : "Ny annons"}
        />
      }
    >
      ...
      <HandleDraftBottomSheet
        ...
        dbDraft={data.getOrCreateDraftProduct}
        product={product}
